...
class EmailAgent:
    ...
    def generate_introduction(
        ...
        article_summaries = "\n".join([
            f"{idx + 1}. "
            f"{article.title if hasattr(article, 'title') else article.get('title', 'N/A')} "
            f"(Score: "
            f"{article.relevance_score if hasattr(article, 'relevance_score') else article.get('relevance_score', 0):.1f}/10)"
...
Generate a greeting and introduction that previews these articles."""
        ...
        try:
            ...
            if not intro.greeting.startswith(
                f"Hey {self.user_profile['name']}"
            ):
                intro.greeting = (
                    f"Hey {self.user_profile['name']}, "
                    f"here is your daily digest of AI news for "
                    f"{current_date}."
                )
            ...
            return EmailIntroduction(
                ...
                introduction=(
                    "Here are the top 10 AI news articles ranked "
